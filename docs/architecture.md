...
| `service` | Which of them a draft goes through, and what to say when it cannot be built. Both entry points ask it, so neither answers the question on its own. |
...
`cmd/trans` is the composition root for *herdr*: it reads the settings,
asks `service` for the one that was chosen, wires the flow to its targets and
starts the program. `cmd/trans-window` is the same thing for Windows, with a
window as the target instead of a pane. Nothing below either of them knows which
...
`HERDR_TRANS_PROVIDER`. The first registered service is the default. A
