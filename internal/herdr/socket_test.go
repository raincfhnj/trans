...
func TestOpeningAPopupAsksForAFloatingPaneOverTheAgent(t *testing.T) {
	t.Parallel()
	server := listen(t, `{"id":"trans","result":{"plugin_pane":{"pane":{"pane_id":"w1:p9"}}}}`)

	pane, err := herdr.NewSocket(server.path).OpenPopup(context.Background(), herdr.Popup{
		PluginID:   "local.trans",
		Entrypoint: "overlay",
		Width:      "70%",
		Height:     14,
		Env:        map[string]string{"HERDR_TRANS_TARGET": "w1:p3"},
	})
	if err != nil {
	...
	if !ok || env["HERDR_TRANS_TARGET"] != "w1:p3" {
...
func TestOpeningAPopupReportsWhatHerdrRefused(t *testing.T) {
	t.Parallel()
	server := listen(t, `{"id":"trans","error":{"code":"invalid_params","message":"popup too large"}}`)
